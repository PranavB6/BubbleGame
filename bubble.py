		...
		for i in range(grid.rows):	
			for j in range(grid._cols):
				...
				if not grid.grid[i][j].exists:
					...
					if int(grid.grid[i][j].pos[0]-BUBBLE_RADIUS)<=int(self.pos[0])<=int(grid.grid[i][j].pos[0]+BUBBLE_RADIUS):
						# print("PASS 1")
						if int(grid.grid[i][j].pos[1]-BUBBLE_RADIUS)<=int(self.pos[1])<=int(grid.grid[i][j].pos[1]+BUBBLE_RADIUS):
							# print("HIT")
							#print(str(i)+","+str(j))
